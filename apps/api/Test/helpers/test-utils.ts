 ...
 * テスト用パスワードハッシュを生成
 */
export async function generateTestPasswordHash(password: string): Promise<string> {
...
export function createMockContext(
	...
	prisma: MockPrismaClient | any = createMockPrisma(),
	jwtSecret: string = 'test-secret-key-for-testing-purposes-only'
) {
	return {
		userId,
 ...
 * テスト用JWTトークンを生成
 */
export function createTestJWT(userId: string, secret: string = 'test-secret-key-for-testing-purposes-only') {
