...
describe('Index DI Tests', () => {
	...
	it('should validate JWT_SECRET length', () => {
		...
		expect(() => envService.validateEnvironment()).toThrow('JWT_SECRET must be set and at least 32 characters');
	});
	...
	it('should reject dev.db in production', () => {
		...
		const envService = new EnvironmentService('/test/path');
		expect(() => envService.validateEnvironment()).toThrow('Production DATABASE_URL must not use dev.db');
	});
	...
	it('should require ALLOWED_WS_ORIGIN in production', () => {
		...
		const envService = new EnvironmentService('/test/path');
		expect(() => envService.validateEnvironment()).toThrow('ALLOWED_WS_ORIGIN must be set in production');
	});
