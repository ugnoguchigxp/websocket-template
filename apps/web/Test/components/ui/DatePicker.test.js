...
vi.mock('react-datepicker', () => ({
    default: ({ selected, onChange, placeholderText, disabled, selectsRange, startDate, endDate, monthsShown, renderCustomHeader }) => (<div data-testid="react-datepicker">
      <input type="text" placeholder={placeholderText} value={selected ? selected.toLocaleDateString() : ''} onChange={(e) => {
        ...
        }} disabled={disabled} data-testid="datepicker-input"/>
      {renderCustomHeader && (<div data-testid="custom-header">
          <button onClick={() => { }} data-testid="year-picker-btn">Year</button>
          <button onClick={() => { }} data-testid="month-picker-btn">Month</button>
        </div>)}
      {selectsRange && (<div data-testid="range-info">
