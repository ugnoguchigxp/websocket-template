...
vi.mock('../../../src/components/notifications/NotificationPanel', () => ({
    NotificationPanel: ({ isVisible }) => (<div data-testid="notification-panel">
      {isVisible ? 'Visible' : 'Hidden'}
    </div>),
}));
vi.mock('../../../src/components/ui/Drawer', () => ({
    default: ({ children, isOpen, onClose, position = 'right' }) => (<div data-testid={`drawer-${position}`} style={{ display: isOpen ? 'block' : 'none' }}>
...
vi.mock('../../../src/components/ui/LanguageSelector', () => ({
    default: ({ className, id }) => (<div data-testid="language-selector" className={className} id={id}>
      Language Selector
    </div>),
}));
vi.mock('../../../src/components/ui/Tooltip', () => ({
    default: ({ text, children }) => (<div data-testid="tooltip" title={text}>
      {children}
    </div>),
}));
vi.mock('../../../src/components/TreeMenu', () => ({
    TreeMenu: ({ menuData, onSelect, showCloseButton, onCloseMenu }) => (<div data-testid="tree-menu">
      <div>Menu Items: {menuData.length}</div>
      {showCloseButton && (<button onClick={onCloseMenu} data-testid="tree-menu-close">
          Close Menu
        </button>)}
    </div>),
}));
vi.mock('../../../src/components/ui/Button', () => ({
    Button: ({ children, onClick, variant, className }) => (<button onClick={onClick} data-testid="button" data-variant={variant} className={className}>
      {children}
    </button>),
}));
