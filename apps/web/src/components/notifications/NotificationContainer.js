...
export const NotificationContainer = ({ maxNotifications = NOTIFICATION_LIMITS.MAX_TOASTS, position = 'top-right', }) => {
    ...
    const containerContent = (<div className={`
        fixed z-50 pointer-events-none
        ${positionClasses[position]}
        w-full max-w-sm
      `} role="region" aria-label="Notifications" aria-live="polite">
			<div className="space-y-2 pointer-events-auto">
				{visibleToasts.map((toast) => (<NotificationToast key={toast.id} notification={toast} onClose={removeNotification} onHide={hideNotification}/>))}
			</div>
