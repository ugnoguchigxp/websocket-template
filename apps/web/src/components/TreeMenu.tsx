...
export const TreeMenu = ({
				...
				{nodes.map(node => {
					...
					return (
						<li key={node.id} className="mb-1">
							<div
								className={`flex items-center justify-between rounded px-2 py-1 transition-colors ${isSelected ? "bg-blue-500 text-white font-bold" : "hover:bg-gray-100"}`}
							>
								{isFolder ? (
									<button
										type="button"
								...
								{node.children && (
									<button
										...
										aria-label={
											isExpanded ? `${displayLabel}を折りたたむ` : `${displayLabel}を展開する`
										}
									>
										{isExpanded ? "▼" : "▶"}
									</button>
	...
	return (
		<div className={`tree-menu ${className}`}>
