...
interface DatePickerProps {
  ...
  disabled?: boolean;
  className?: string;
  monthsShown?: number; // Number of months to display (1, 2, or 3)
}
...
const DatePicker: React.FC<DatePickerProps> = ({
  ...
  const closeTimer1Ref = useRef<NodeJS.Timeout | null>(null);
  const closeTimer2Ref = useRef<NodeJS.Timeout | null>(null);
  const [showYearPicker, setShowYearPicker] = useState(false);
  const [showMonthPicker, setShowMonthPicker] = useState(false);

  // Generate years array (100 years centered on current year)
  const years = useMemo(
    () => Array.from({ length: 100 }, (_, i) => getYear(new Date()) - 50 + i),
    []
  );
  ...
  return (
    <div className={`datepicker-wrapper ${className}`}>
      {selectsRange ? (
        ...
        <ReactDatePicker {...commonProps} selected={value || undefined} onChange={handleChange} />
      )}
    </div>
  );
};
