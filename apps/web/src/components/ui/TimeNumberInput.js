...
export const TimeNumberInput = ({ value, onChange, label = '時刻を入力 (4桁)', placeholder = 'タップして入力', disabled = false, className = '', }) => {
    const [modalOpen, setModalOpen] = useState(false);
    const [inputValue, setInputValue] = useState('');
    const [errorMessage, setErrorMessage] = useState('');
    const formatTimeDisplay = (numStr) => {
    ...
    return (<>
			<Input type="text" value={displayValue} placeholder={placeholder} onClick={handleOpen} readOnly disabled={disabled} className={className}/>
			<KeypadModal open={modalOpen} title={label} onClose={handleClose} displayContent={<div className="text-3xl font-bold text-gray-900">
					...
					</div>} errorMessage={errorMessage} onNumberClick={handleNumberClick} onBackspace={handleBackspace} onClear={handleClear} onConfirm={handleConfirm}/>
		</>);
};
