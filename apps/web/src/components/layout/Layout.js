...
const Layout = () => {
    ...
    return (<div className="overflow-hidden">
			{!isMobile && !isTreeMenuOpen && (<div className="fixed left-0 z-40 flex items-center top-[124px] h-10 w-[30px]">
					<button id="bookmark-toggle-open" aria-label="Open menu" className="bg-blue-600 text-white rounded-r-lg shadow p-3 flex flex-col items-center hover:bg-blue-700 focus:outline-none w-[30px] h-10 justify-center" onClick={() => setIsTreeMenuOpen(true)}>
			...
			<nav className="bg-white">
						...
						<div className="flex items-center space-x-4">
							...
							<div className="relative">
								<Tooltip text={t('click_for_details', 'クリックで詳細表示')}>
									<button onClick={() => setDropdownOpen((open) => !open)} className="flex items-center justify-center w-12 h-12 rounded-full bg-gray-200 hover:bg-gray-300 focus:outline-none relative" aria-label="User menu" type="button">
				...
				<div className="md:hidden flex justify-between items-center px-4 py-2 border-t bg-white">
					<Tooltip text={t('menu', 'メニュー')}>
						<button onClick={() => setIsMobileMenuOpen(true)} className="flex items-center justify-center w-10 h-10 rounded-full bg-gray-200 hover:bg-gray-300 focus:outline-none" aria-label="Menu">
			...
			<div className="flex w-full">
				{!isMobile && (<div className="relative h-full bg-white transition-all duration-300 ease-in-out flex-shrink-0 overflow-hidden" style={{
                ...
                pointerEvents: isTreeMenuOpen ? 'auto' : 'none',
						...
						<div className="w-full h-full transition-transform duration-300 ease-in-out" style={{
                transform: isTreeMenuOpen ? 'translateX(0)' : 'translateX(-100%)',
                transition: 'transform 0.3s cubic-bezier(0.4,0,0.2,1)',
            }}>
							<TreeMenu menuData={menuData} onSelect={() => { }} showCloseButton={true} onCloseMenu={() => setIsTreeMenuOpen(false)}/>
						</div>
					</div>)}
