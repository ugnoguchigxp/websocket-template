...
const sampleNotifications: DemoNotification[] = [
	{
		type: 'info',
		title: 'システム情報',
...
export function NotificationDemoPage() {
	const [selectedDemo, setSelectedDemo] = useState<DemoNotification>(sampleNotifications[0]!);
	...
	const handleDemoNotification = (demo: DemoNotification) => {
	...
	const handleConvenienceMethod = (method: string) => {
		...
		<div className="p-6 space-y-6 max-w-4xl mx-auto">
			<div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
				...
				<div className="space-y-4">
							...
							return (
								...
								>
									<div className="flex items-start space-x-3">
										<Icon
											className={`w-5 h-5 mt-0.5 ${selectedDemo === demo ? '' : 'text-gray-500'}`}
										/>
					...
					<button
						onClick={() => handleDemoNotification(selectedDemo)}
					...
					<div className="space-y-4">
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div>
								...
								<input
									...
									maxLength={NOTIFICATION_LIMITS_TEXT.TITLE_MAX}
								/>
							</div>
						...
						<div>
							...
							<textarea
								...
								maxLength={NOTIFICATION_LIMITS_TEXT.MESSAGE_MAX}
							/>
				...
				<div className="border-t pt-6 mt-6">
					...
					<div className="flex flex-wrap gap-2">
						<button
							onClick={() => handleConvenienceMethod('success')}
							className="px-3 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors text-sm"
						>
							showSuccess()
						</button>
						<button
							onClick={() => handleConvenienceMethod('error')}
							className="px-3 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition-colors text-sm"
						>
							showError()
						</button>
						<button
							onClick={() => handleConvenienceMethod('warning')}
							className="px-3 py-2 bg-yellow-600 text-white rounded-md hover:bg-yellow-700 transition-colors text-sm"
						>
							showWarning()
						</button>
						<button
							onClick={() => handleConvenienceMethod('info')}
