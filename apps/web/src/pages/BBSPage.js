...
export function BBSPage() {
    ...
    const comments = api.posts.comments.list.useQuery(selectedPostId ? { postId: selectedPostId } : undefined, { enabled: !!selectedPostId });
    // New Post modal
    ...
    return (<div className="bg-gray-100 min-h-full">
			<main className="max-w-6xl mx-auto px-4 py-6">
				...
				<div className="bg-white border border-gray-400 mb-6">
					...
					<div className="divide-y divide-gray-300">
						{posts.isLoading ? (<div className="px-4 py-8 text-center font-mono text-gray-500">{t('loading')}</div>) : posts.data?.items.length ? (<table className="w-full font-mono text-sm">
								...
								<tbody>
									{posts.data.items.map((post, index) => (<tr key={post.id} className={`hover:bg-blue-50 cursor-pointer ${selectedPostId === post.id ? 'bg-blue-100' : ''}`} onClick={() => setSelectedPostId(post.id)}>
							...
							</table>) : (<div className="px-4 py-8 text-center font-mono text-gray-500">{t('no_posts')}</div>)}
					</div>
				</div>

				{/* Selected Thread */}
				{selectedPost && (<div className="bg-white border border-gray-400">
						<div className="bg-gray-200 border-b border-gray-400 px-4 py-2">
						...
						<div className="divide-y divide-gray-200">
							{comments.isLoading ? (<div className="px-4 py-8 text-center font-mono text-gray-500">{t('loading')}</div>) : comments.data?.length ? (comments.data.map((comment, index) => (<div key={comment.id} className="px-4 py-4">
										<div className="flex items-baseline gap-4 mb-2 font-mono text-sm text-gray-600">
											<span>
									...
									</div>))) : (<div className="px-4 py-4 font-mono text-gray-500">{t('no_comments')}</div>)}
						</div>
						...
						<div className="border-t border-gray-400 bg-gray-50 px-4 py-4">
							...
							<div className="space-y-2">
								<Textarea placeholder={t('comment_placeholder')} value={commentBody} onChange={(e) => setCommentBody(e.target.value)} className="font-mono" rows={4}/>
								<Button onClick={() => selectedPostId && addComment.mutate({ postId: selectedPostId, body: commentBody })} disabled={!commentBody || addComment.isPending} className="font-mono">
									{t('post')}
								</Button>
							</div>
						</div>
